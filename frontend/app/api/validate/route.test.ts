...
`;
      ...
      const payload = await response.json() as { ok: boolean; plan?: { nodeCount: number; layerCount: number; entrypoint: string } };
      expect(response.status).toBe(200);
      expect(payload).toMatchObject({
        ok: true,
        plan: { nodeCount: 4, layerCount: 3, entrypoint: "output" },
      });
    } finally {
      if (previous === undefined) delete process.env.HARNEST_FILE;
      else process.env.HARNEST_FILE = previous;
