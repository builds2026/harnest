...
| Studio UX | 구현됨 | 첫 방문은 결과·필요 서비스·sample input이 보이는 5개 Recipe launchpad다. 편집 화면은 Build/Tools/Skills/Services/Recipes, Configure, Setup/Try/Tests/Compare/Activity/YAML로 정리했다. 저장과 runtime validation은 debounce로 자동 진행하고 상단에는 Recipe → Services → Ready → Result와 한 개의 다음 행동만 표시한다. Connection은 한 번의 Connect로 저장·인증/승인·test까지 진행한다. Try는 답을 먼저 표시하고 기술 metadata를 접는다. Tests는 일반 case를 추가·수정·삭제하고 성공률·case latency를 표시하며 고급 assertion은 손실 없이 YAML에 유지한다. approval은 사람이 이해할 수 있는 권한 설명과 advanced digest를 분리한다. |
