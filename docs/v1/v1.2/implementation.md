...
| MCP OAuth | 구현됨/외부 미검증 | metadata discovery, DCR, PKCE/state, loopback callback, issuer/resource-bound token, refresh, insufficient-scope 재동의, revoke/reauth를 제공하고 하나의 local protocol E2E로 검증했다. 실제 외부 authorization server consent UX는 credential 부재로 미검증이다. |
...
| Studio UX | 구현됨 | Components/Tools/Skills/Connections/Templates palette, 검색·분류·최근·즐겨찾기, compatible picker, Inspector, onboarding, Run/Test/Trace, Advanced YAML이 있다. Template은 기존 compatible Connection을 자동 배선하고, 기존 YAML의 선언된 미완료 ID도 필요한 wizard를 차례로 연다. Connection은 한 번의 Connect로 저장·인증/승인·test까지 진행한다. |
...
| 전체 검증 | 구현됨/외부 일부 미검증 | unit/integration/protocol, local OAuth, fake container engine, local Search fixture, CLI lifecycle, production build와 in-app Studio E2E가 있다. 실제 외부 OpenAI/Anthropic/Firecrawl/SearXNG/OAuth 및 실행 중인 Docker daemon은 현재 환경의 credential/service 부재로 미검증이다. 자세한 내용은 `tests.md`와 `remaining-issues.md`를 따른다. |
